...
struct StoryChoiceButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .shadow(color: AppTheme.gold.opacity(configuration.isPressed ? 0.30 : 0.20), radius: configuration.isPressed ? 5 : 11, x: 0, y: configuration.isPressed ? 2 : 6)
    ...
    private func background(pressed: Bool) -> some View {
        RoundedRectangle(cornerRadius: AppTheme.buttonRadius, style: .continuous)
            ...
            .overlay(RoundedRectangle(cornerRadius: AppTheme.buttonRadius, style: .continuous).stroke(AppTheme.gold.opacity(pressed ? 0.8 : 0.35), lineWidth: pressed ? 2 : 1).blur(radius: reduceMotion ? 0 : 1))
    }
}
